# From buguroo's "more strategies" for hypothesis package:
# https://github.com/buguroo/more_strategies
# Released under the GNU General Public License v3.0, which can be found in the repo above.
...
def url(schemes=[], userpass=False, port=False, url=False, query=False,
    ...
    return st.fixed_dictionaries(d).map(to_url).filter(max_len())